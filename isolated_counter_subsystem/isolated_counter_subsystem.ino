/*
 * Driver for the isolated counter subsystem.
 */
...
#define OUT_ENABLE_B_NOT_FUTURE A1
#define OUT_PULSE_A A2
#define OUT_PULSE_B_FUTURE A3
#define OUT_CCLR_A_NOT A4
#define OUT_CCLR_B_NOT_FUTURE A5
...
void setup() {
  ...
  // OUT_ENABLE_B_NOT_FUTURE
  pinMode(OUT_PULSE_A, OUTPUT);
  // OUT_PULSE_B_FUTURE
  pinMode(OUT_CCLR_A_NOT, OUTPUT);
  // OUT_CCLR_B_NOT_FUTURE
  ...
  // OUT_ENABLE_B_NOT_FUTURE
  digitalWrite(OUT_PULSE_A, LOW);
  // OUT_PULSE_B_FUTURE
  digitalWrite(OUT_CCLR_A_NOT, HIGH);
  // OUT_CCLR_B_NOT_FUTURE
...
void clear_counters() {
  long_descending_pulse(OUT_CCLR_A_NOT);
  rising_pulse(OUT_RCLK_A);
  // TODO: Counter B -- or maybe not.
}
...
void loop() {
  ...
  uint32_t loaded_value = load_counter_with_test_value(value_index, OUT_PULSE_A);
  ...
  Serial.println(read_counter(OUT_ENABLE_A_NOT), HEX);
